...
add_text('「日本酒は辛口がうまい」―― 居酒屋でもよく聞くこのフレーズ。しかし最近、日本酒レビューサイトのランキング上位を占めるのは甘口でフルーティな銘柄ばかりだ。辛口の時代は本当に終わったのか？ 国立国会図書館の出版物データ、日本酒レビューサイトSAKETIMEの18万件のレビュー、そしてGoogle Trendsの検索データを横断分析し、140年のスパンで答えを探った。')
...
add_text('そこで、SAKETIMEの全5,386銘柄・約18万件のレビューをスクレイピングし、体系的に検証することにした。さらに、「辛口信仰」がそもそもいつ始まったのかを、国立国会図書館の出版物データやGoogle Trendsで140年のスパンで追ってみた。')
...
add_bullet('甘口率（甘い+1以上）: Top10で67.3%、101-500位で35.4% → 上位ほど甘口は確かに正しい')
...
add_text('約18万件のレビューテキストから味わいキーワードの出現率をランキング帯別に比較すると、もう一つの特徴が見えてきた。')
...
add_text('日本酒の味わいトレンドには「二重構造」が存在している。愛好家コミュニティではとっくに甘口・フルーティへのシフトが起きているのに、一般消費者の検索行動ではまだ「辛口」が軸なのだ。')
