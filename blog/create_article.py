...
add_text('「日本酒は辛口にかぎる」―― 以前は居酒屋でもよく聞かれたこのフレーズ。しかし最近、よく聞く銘柄は甘口でフルーティなものばかり。辛口の時代は本当に終わったのか？ 国立国会図書館の出版物データ、日本酒レビューサイトSAKETIMEの約37万件のレビュー、そしてGoogle Trendsの検索データを横断分析し、140年のスパンで答えを探った。')
...
add_text('きっかけはYouTubeの日本酒動画だった。その動画では、日本最大級の日本酒レビューサイト「SAKETIME」のランキングを見て、「上位に入る日本酒は、甘旨（あまうま）ジューシーものばかり」という結論に達していた。')

add_text('十四代、新政、而今、花陽浴、どの銘柄も一言で表すのは難しい深みを持っているが、直感的にはそうかもしれない。しかし、本当にデータで裏付けられるのだろうか？ そして、もしランキング上位が甘口ばかりだとしたら、あの「辛口がいい酒」という長年の常識はどうなったのか？')
...
add_text('明治時代（1880年代）には、「甘口」が出版物で圧倒的に多く使われていた。文献ではよく指摘されることだが、江戸時代の日本酒の味わいの語りはもともと「甘口」が中心だった。そのような傾向がデータに現れている可能性がある。')

add_text('「甘口」はその後100年かけて緩やかに減少し、代わりに1970年代から「辛口」が急上昇する。同時期に日本酒固有の語である「淡麗」も上昇していることから、これは日本酒の「淡麗辛口ブーム」の到来を反映していると推測できる。')

add_text('「辛口」信仰を戦後の三増酒（米不足から醸造アルコール、糖類、酸味料を加え、原料米の約3倍量に増量したお酒、甘くベタつく味わいから低品質な酒の代名詞）へのカウンターとする議論も聞かれるが、少なくとも第二次世界大戦直後に「辛口」が文献レベルで増えたという証拠は見つけられなかった。')
...
add_text('では、現在の日本酒レビューサイトではどうなっているのか。SAKETIMEの全5,386銘柄・約37万件のレビューを分析した。')

doc.add_heading('構造化データ：上位ほど甘口、しかし「濃醇」ではない', level=2)
...
add_text('YouTube動画の「甘口」という指摘は正しかったが、「旨味」、「ジューシー」についてはある程度慎重になる必要があるように思う。「濃醇」＝「旨味」ではない。ここではボディが重いか軽いかということだが、軽快なものというのは水のようにするする呑めるものと考えた時には、一定程度の関連をここにみても良いだろう。これは上位銘柄は甘口だが、ボディは重厚ではなく普通〜やや軽めなのだ。')
...
add_text('花陽浴のフルーティ率37%、陽乃鳥・宮寒梅の甘口率44%が際立つ。一方、ランキング1位の十四代は辛口率わずか1.8%だが、甘口率もそれほど高くない、突出した偏りがなくバランスの良さが特徴だ。')
...
add_quote('甘口は王道として定着した。しかし辛口も復権しつつある可能性も。一方的なシフトではなく、多様化が進んでいるのかもしれない。')

add_separator()

# ===== 第4章 =====
doc.add_heading('第4章：「辛口の時代」は終わったのか？ ― 意外な答え', level=1)

add_text('SAKETIMEのデータからは、甘口の定着と辛口復権の兆しが見えた。では、もっと広い視点で見るとどうか。一般消費者はどう動いているのだろう。')
...
# ===== 参考：SAKETIMEランキングの全体像 =====
doc.add_heading('参考', level=1)

doc.add_heading('SAKETIMEランキングの全体像', level=2)

add_text('SAKETIMEには5,386銘柄がランキングされている。評価スコアの平均は3.18（5点満点）で、大半が3.0前後に集中。注目すべきは、レビュー数（対数スケール）と評価の相関が0.888と非常に高いこと。多くのレビューを集める銘柄ほど高評価を得ている。')

add_image('fig1_rating_vs_reviews.png', '評価スコア vs レビュー数。上位10銘柄をラベル表示。')

doc.add_heading('特定名称による評価の違い', level=2)

add_image('fig10_price_specname.png', '特定名称別の平均評価')

add_text('特定名称酒のカテゴリーが評価にどう影響するかを見ておこう。純米大吟醸（4.16）が最も高く、純米吟醸（4.05）が続く。一方、本醸造（3.79）や普通酒（3.75）は相対的に低い。精米歩合が高く手間をかけた酒ほど高評価という、ある意味で順当な結果だ。ただし差は0.4ポイント程度であり、特定名称だけで評価が決まるわけではない。')

doc.add_heading('さけのわデータによる独立検証', level=2)

add_text('本文の分析はすべてSAKETIMEのデータに基づいている。結論の妥当性を検証するため、独立したデータソースである「さけのわ」のデータでクロスチェックを行った。')
...
add_image('fig12_sakenowa_analysis.png', 'さけのわランキング帯別のフレーバータグ出現率（左）とフレーバーチャート6軸平均（右）')

add_text('フレーバーチャート6軸を見ると、上位銘柄は「華やか」が高く（1-10位: 0.50 → 圏外: 0.35）、「重厚」が低い（1-10位: 0.24 → 圏外: 0.35）。SAKETIMEで見た「上位は芳醇ではなく軽め」という結論と完全に一致する。')

add_text('フレーバータグでは、辛口タグが1-10位で0%なのに対し51-100位では78%に達する。フルーティは1-10位で100%、圏外で33%。独立したデータソースで、SAKETIMEの結論 ― 上位銘柄は「華やかでフルーティ、軽やかでバランスが良い」、辛口や濃厚は上位ほど少ない ― が裏付けられた。')
