...
`@namazakepaul` という、海外在住の日本酒愛好家の方から「**The rise of acidity is interesting too**（酸味の台頭も面白いよ）」というコメントをいただいた。私の返信はこんな趣旨だった。「**同意です。ただ酸味（acidity）は辛口（karakuchi）のような確立された日本酒の語彙ではないので、長期トレンドが追いにくい。SAKETIME の構造化評価も甘辛・濃淡の2軸しかなく、酸味専用の指標がないから構造的な分析にも限界がある。ただ、レビューのテキストを実際に解析すれば何か出てくるかもしれない。やってみて、見えたらまた共有しますね**」。
...
確かに、日本酒には「酸度」という公式の指標がある。中和滴定で測られる、ラベルにも書かれる数字だ。**「辛口」は感覚的・主観的な表現に過ぎないのに、「酸味」のほうにはむしろ計測可能な実体がある**。それなのに、レビューの言葉のなかで「酸味」は「辛口」「甘口」より圧倒的にマイナーな存在に見える。客観的な指標があるはずの「酸」のほうが、なぜ語られないのか。あるいは、語られ始めているとしたら、いつから、どんな速さで、どんな種類の酸が増えているのか。

書き終えてからずっと引っかかっていた **「辛口が物差しから降りたなら、何が代わりに上がってきたのか」** という問いと、Paul さんの「酸度こそ客観的だ」という指摘が、頭の中で繋がった。
...
![6カテゴリ味覚トレンド（全銘柄）](../data/acidity_six_categories_trend.png)
*図1: 全銘柄レビューにおける味覚キーワード出現率の推移。酸味系は2014年 7.2% → 2025年 27.5%（3.8倍）に上昇。*

結果はかなり明瞭だった。

| カテゴリ | 2014 | 2025 | 差分 | 倍率 |
|---|---|---|---|---|
| 甘口系 | 6.5% | 30.0% | +23.4pt | 4.58× |
| **酸味系** | **7.2%** | **27.5%** | **+20.3pt** | **3.82×** |
| フルーティ系 | 6.6% | 20.5% | +13.9pt | 3.11× |
| 芳醇系 | 12.0% | 23.4% | +11.4pt | 1.95× |
| 軽快系 | 9.8% | 20.1% | +10.4pt | 2.06× |
| 辛口系 | 7.7% | 13.3% | +5.7pt | 1.74× |
...
![酸タイプ別の出現率推移（全銘柄）](../data/acidity_by_type_trend.png)
*図2: 4つの酸タイプ別の出現率推移。乳酸系が 2014年 0.12% → 2026年 2.50% と21倍、生酛・山廃の再評価が読み取れる。*

| 酸タイプ | 2014 | 2021 | 2026 | 倍率 |
|---|---|---|---|---|
| 乳酸系 | 0.12% | 3.08% | 2.50% | **21.0×** |
| クエン酸系 | 0.83% | 3.71% | 3.87% | 4.6× |
| リンゴ酸系 | 0.12% | 0.53% | 0.40% | 3.4× |
| 酒石酸系 | 1.49% | 3.43% | 2.61% | 1.8× |
...
クエン酸系の絶対値は最大で、2026年に 3.87%。これは白麹を採用する蔵（富久長・黒兜など）の台頭と連動している。後で詳しく見るが、Google Trends の「白麹」検索量とほぼ完全に同期している。
...
新政、仙禽、土田、富久長。それぞれの蔵が、4つの酸タイプのうちどれを得意とするかを、レビュー内の酸味系語彙の出現率で可視化してみた。

![先行銘柄 × 酸タイプの出現率プロファイル](../data/acidity_brand_type_heatmap.png)
*図3: 酸味先行銘柄ごとの酸タイプ別出現率（全期間平均）。各蔵が異なる酸の方向に特化していることが見える。*

| 銘柄 | 乳酸 | クエン酸 | リンゴ酸 | 酒石酸 | キャラクター |
|---|---|---|---|---|---|
| 土田 | **13.1%** | 4.6 | 0.0 | 5.1 | 乳酸系の専門家（山廃・生酛） |
| 飛鸞 | 9.4 | **13.5%** | 0.3 | 3.7 | 乳酸 + クエン酸のハイブリッド |
| 富久長 | 2.2 | **8.6%** | 0.0 | 4.7 | 白麹によるクエン酸特化 |
| 新政 | 3.8 | **8.5** | 0.4 | **6.5** | クエン酸 + 酒石酸の多角型 |
| 仙禽 | 6.2 | 7.1 | 0.3 | 5.4 | バランス型（全方位高め） |
| 風の森 | 1.7 | 3.0 | 0.4 | 4.1 | クリーン系・控えめ |
| 而今 | 1.8 | 1.5 | 1.3 | 1.1 | 全体的に淡い（繊細・上品） |
| 全銘柄平均 | 2.1 | 3.2 | 0.3 | 2.9 | — |

特筆すべきは **土田の乳酸系 13.1%（全国平均 2.1%の 6倍）** と、**富久長のクエン酸系 8.6%（全国平均の 2.7倍）**。どちらも酒蔵の公式な方針（土田は山廃・生酛、富久長は「海風土」など白麹採用）と、レビュー言語の現れ方が一致している。

新政が興味深いのは、生酛回帰で有名な蔵でありながら、データ上はクエン酸系 (8.5%) と酒石酸系 (6.5%) のほうが乳酸系 (3.8%) より高く出る点だ。これは新政の幅広い商品ライン（No.6・PRIVATE LAB シリーズ・陽乃鳥など）の多様性を反映しているはずで、「生酛蔵 = 乳酸系一辺倒」では捉えきれない実態が見える。

仙禽がほぼ全方位で平均を上回っているのは、酸味系日本酒のパイオニアとして全タイプの表現を試みてきた蔵の幅広さを示しているように思う。一方、而今が全タイプで全国平均を下回るのは、「繊細・上品」を貫く酒質設計の現れだろう。前作のラダーチャートで而今が辛口・甘口どちらにも振り切らないバランス型として出てきたのとも整合する。
...
![Google Trends: 酸味関連クエリ](../data/acidity_gt_acid_specific.png)
*図4: Google Trends で見た「日本酒 酸味」「白麹」「生酛」の月次推移。生酛・白麹は 2014→2026 で 2〜4倍に上昇。*

| クエリ | 2014年平均 | 2020年平均 | 2026年平均 | 倍率 |
|---|---|---|---|---|
| 生酛 | 17.6 | 34.0 | 71.2 | **4.0×** |
| 白麹 | 25.2 | 34.9 | 54.2 | 2.2× |
| 日本酒 酸味 | 0.0 | 0.0 | 3.8 | 検索量小 |
...
「甘口」と「フルーティ」だけでは、現代の上位銘柄の特徴を語り切れない。新政の生酛、仙禽のリンゴ酸、土田の山廃、富久長の白麹。同じ「酸味のある日本酒」と語られる4蔵は、それぞれ違う化学反応で違う酸を作っていて、レビュー言語の中でもそれぞれ違う pattern を残している。データを4つに分けて眺めて初めて、新政と仙禽の違い、土田と富久長の違いがはっきり見える。
